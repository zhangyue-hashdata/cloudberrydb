...
CREATE OR REPLACE FUNCTION "get_pax_table_stats"(table_name text)
  RETURNS TABLE("ptstatistics" pg_ext_aux.paxauxstats) AS $BODY$
    DECLARE
    subquery  varchar;
    pre_sql   varchar;
    table_oid Oid;
    ...
    pre_sql:='select oid from pg_class where relname='''||table_name||'''';
    EXECUTE pre_sql into table_oid;
    subquery := 'select ptstatistics from gp_dist_random(''pg_ext_aux.pg_pax_blocks_'||table_oid||''')';
    RETURN QUERY execute subquery;
    END
    $BODY$
  LANGUAGE plpgsql;