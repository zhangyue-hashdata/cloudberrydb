...
std::vector<std::string> LocalFileSystem::ListDirectory(const std::string &path) const {
  DIR *dir;
...
void LocalFileSystem::CopyFile(const std::string &src_file_path, const std::string &dst_file_path) const {
...
void LocalFileSystem::DeleteDirectory(const std::string &path, bool delete_topleveldir) const {
  cbdb::PathNameDeleteDir(path.c_str(), delete_topleveldir);
}

std::string LocalFileSystem::BuildPaxDirectoryPath(RelFileNode rd_node, BackendId rd_backend) const {
  std::string dirpath;
...
std::string LocalFileSystem::BuildPaxFilePath(const Relation rel, const std::string &block_id) const {
  std::string filepath;
...
namespace paxc {
static void DeletePaxDirectoryPathRecursive(const char *path,
           const char *toplevel_path, bool delete_topleveldir,
...
// DeletePaxDirectoryPath: Delete a directory and everything in it, if it exists.
// parameter dirname IN directory to delete recursively.
// parameter reserve_topdir IN flag indicate if reserve top level directory.
void DeletePaxDirectoryPath(const char *dirname, bool delete_topleveldir) {
  ...
  DeletePaxDirectoryPathRecursive(dirname, dirname, delete_topleveldir, UnlinkIfExistsFname, false, LOG);
}

// BuildPaxDirectoryPath: function used to build pax storage directory path following pg convension,
...
// BuildPaxFilePath: function used to build pax storage directory path following pg convension,
// for example base/{database_oid}/{blocks_relid}_pax.
// parameter rel IN Relation information.
// parameter block_id IN micro-partition block id.
// return palloc'd pax storage directory path.
  ...
  while ((de = ReadDirExtended(dir, path, elevel)) != NULL) {
    ...
    switch (get_dirent_type(subpath, de, process_symlinks, elevel)) {
      case PGFILETYPE_REG:
        ...
        break;
      case PGFILETYPE_DIR:
        DeletePaxDirectoryPathRecursive(subpath, toplevel_path, delete_topleveldir, action, false, elevel);
        break;
      default:
        break;
    }
