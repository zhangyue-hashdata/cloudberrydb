...
#include "storage/pax_arrow_wrapper.h"
...
// extern std::shared_ptr<arrow::DataType> TypeOidToDataType(Oid typid);
...
class ParallelScanDesc : public std::enable_shared_from_this<ParallelScanDesc> {
 public:
  ...
  inline ParallelIterator<MicroPartitionMetadata> *Iterator() { return iterator_.get(); }
  ...
  void Release();
  const std::shared_ptr<arrow::Schema> &TableSchema() const { return table_schema_; }
  std::shared_ptr<arrow::Schema> &TableSchema() { return table_schema_; }
  const std::shared_ptr<arrow::Schema> &ScanSchema() const { return scan_schema_; }
 ...
 private:
  ...
  void TransformFilterExpression(const std::shared_ptr<arrow::dataset::ScanOptions> &scan_options,
  ...
  std::vector<ScanKeyData> filter_scan_keys_;
...
class PaxFragmentInterface : public arrow::dataset::FragmentInterface {
 public:
  ...
  arrow::Result<arrow::RecordBatchIterator> ScanBatchesAsyncImpl(const std::shared_ptr<arrow::dataset::ScanOptions>& options) override;
  std::string type_name() const override { return "pax-fragment"; }
  arrow::Result<std::shared_ptr<arrow::RecordBatch>> Next();

  int BlockNum() const { return block_no_; }
  const std::shared_ptr<arrow::Schema> &ScanSchema() const { return scan_desc_->ScanSchema(); }
  const std::vector<int> &ScanColumns() const { return scan_desc_->ScanColumns(); }
  void Release() override;
