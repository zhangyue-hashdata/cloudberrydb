...
void File::ReadN(void *ptr, size_t n) {
  auto num = Read(ptr, n);
  CBDB_CHECK((static_cast<size_t>(num)) == n, cbdb::CException::ExType::kExTypeIOError);
}
...
void File::PReadN(void *buf, size_t count, off_t offset) {
  ...
  CBDB_CHECK((static_cast<size_t>(num)) == count, cbdb::CException::ExType::kExTypeIOError);
}
